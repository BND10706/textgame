...
# Your player looks good. You added addtional things.But that is totally up to you.
# now this down below is not needed, unless your just using it to print.
...
# Ok, again, alot of extra things, so, totally up to you.
class Monster:
    def __init__(self, name, max_health, health, max_strength, strength, special):
        self.name = name
        # unless you plan on giving the monster the ability to heal, you dont need to give them max health.
        self.max_health = max_health
        self.health = health
        self.max_strength = max_strength
        self.strength = strength
...
# These monsters are not needed, you will generate the monster at random....
...
# OK... so you have the right idea... but the generate monster is a function


def generate_monster():
    monster_types = [
        ("Ogre", 25, 22, 15, 10, 1),
        ("Goblin", 50, 45, 200, 176, 1),
        ("Troll", 30, 25, 35, 35, 5),
    ]


# This is how you create the monster types.Now what you have to do is intialize it at the random.
# sooo like this.
name, max_health, health, max_strength, strength, special = random.choice(monster_types)
# then you just need to return.....  before the return you can just print for now.  like you did...if you want do to that,
# assign it to a variable first. I would not worry about it. i would just do a return then you can call the function.
# Return ClassName(all the attribuetes lsited here that you want to return(so probably everything))
